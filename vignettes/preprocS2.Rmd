...
We then used the R package [`sen2r`](https://sen2r.ranghetti.info/) in order to identify S2 product available for a specific date of acquisition, and download the corresponding Sentinel-2 SAFE archive. 

# Here, level-2A images (atmospherically corrected with Sen2Cor) can be downloaded directly from  [SciHub](https://scihub.copernicus.eu) with sen2r.
...
list_safe <- s2_list(spatial_extent = sf::st_read(dsn = path_vector), 
                     time_interval = time_window, server = 'gcloud')
...
You can also use the function `get_S2_L2A_Image` from `preprocS2` in order to download this image. This function uses the `s2_download` from `sen2r`. 

WARNING: starting from sen2r version 1.6.0, [scihub is no longer avaibale from sen2r](https://github.com/ranghetti/sen2r/issues/464). Users are required to download S2 images from the Google Cloud Sentinel-2 bucket. `GoogleCloud = TRUE` is the default option, and `GoogleCloud = FALSE` stops the process. 
...
Once reflectance data is written as a raster file, the R package [`spinr`](https://gitlab.com/jbferet/spinr) can be used for the computation of various spectral indices and biophysical variables direcrtlty from a raster stack corresponding to Level-2A reflectance. 
