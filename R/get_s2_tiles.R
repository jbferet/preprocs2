...
#'
#' @return s2_tiles list of tiles corresponding to plots
...
get_s2_tiles <- function(plots, dsn_bbox, output_dir, site_name = NULL,
  ...
  if (!file.exists(s2_tiles_path) | overwrite == TRUE){
    ...
    if (inherits(x = plots, what = 'list')){
      s2_tiles <- lapply(X = plots, FUN = get_plot_tile, S2tilingGridsub)
    } else if (inherits(x = plots[[1]], what = c('sf', 'sfc'))){
      s2_tiles <- get_plot_tile(aoi = plots, S2_footprint = S2tilingGridsub)
    } else {
      message('get_s2_tiles expects simple feature: sf, sfc or list of sf/sfc')
      stop()
    }
    # save S2 tiles corresponding to each cell grid
    saveRDS(object = s2_tiles, file = s2_tiles_path)
