...
#' @export
#'
mosaic_tiles <- function(pattern, dir_path, vrt_save, siteName, overwrite = FALSE){
  # create vrt
  listfiles <- list.files(dir_path, pattern = pattern, full.names = TRUE)
  output_vrt_path <- file.path(getwd(), paste0(siteName, '_', pattern,'_mosaic.vrt'))
  if (!file.exists(output_vrt_path))
    v <- terra::vrt(x = listfiles, filename = output_vrt_path)
  # create tiff from vrt
  tif_path <- file.path(dir_path, paste0('_', siteName, '_', pattern,'_mosaic.tiff'))
  ...
  output_vrt_path2 <- file.path(vrt_save, paste0(siteName, '_', pattern,'_mosaic.vrt'))
