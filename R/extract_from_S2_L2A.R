...
extract_from_S2_L2A <- function(Path_dir_S2,
  ...
  if (!is.null(path_vector)){
    ...
    path_vector_reproj <- paste(tools::file_path_sans_ext(path_vector),
                                '_reprojected.gpkg',sep = '')
  ...
  S2_Stack <- read_s2_bands(S2_Bands = S2_Bands,
                            aoi = aoi,
                            resolution = resolution,
  ...
  if (!is.null(S2_Bands$S2Bands_10m) & !is.null(S2_Bands$S2Bands_20m)){
    ...
    if (!is.na(match('B02',NameBands)))
      S2Bands <- c("B02", "B03", "B04", "B05", "B06", "B07", "B08",
                   "B8A", "B11", "B12", "Cloud")
    if (!is.na(match('B2',NameBands)))
      S2Bands <- c("B2", "B3", "B4", "B5", "B6", "B7", "B8", "B8A",
                   "B11", "B12", "Cloud")
